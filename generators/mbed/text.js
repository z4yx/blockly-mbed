...
/**
 * @fileoverview mbed code generator for the text blocks.
 *     Partially implements the mbed Serial interface as described in:
...
goog.provide('Blockly.mbed.text');
 ...
 */
Blockly.mbed['text'] = function(block) {
  var code = Blockly.mbed.quote_(block.getFieldValue('TEXT'));
 ...
 * String construction info: http://mbed.cc/en/Reference/StringConstructor
 * mbed code: loop { "String(X)" + ... + "String(Y)" }
 * @param {!Blockly.Block} block Block to generate the code from.
 * @return {array} Completed code with order of operation.
 */
Blockly.mbed['text_join'] = function(block) {
  var code;
  if (block.itemCount_ == 0) {
    return ['""', Blockly.mbed.ORDER_ATOMIC];
  } else if (block.itemCount_ == 1) {
    var argument0 = Blockly.mbed.valueToCode(block, 'ADD0',
    ...
    for (var n = 0; n < block.itemCount_; n++) {
      argument = Blockly.mbed.valueToCode(
          block, 'ADD' + n, Blockly.mbed.ORDER_NONE);
      if (argument == '') {
        code[n] = '""';
      } else {
 ...
 * String constructor info: http://mbed.cc/en/Reference/StringConstructor
 * mbed code: loop { X += String(Y) }
 * @param {!Blockly.Block} block Block to generate the code from.
 ...
 * String length info: http://mbed.cc/en/Reference/StringLength
 * mbed code: loop { String(X).length() }
 * @param {!Blockly.Block} block Block to generate the code from.
 * @return {array} Completed code with order of operation.
 */
Blockly.mbed['text_length'] = function(block) {
 ...
 * String length info: http://mbed.cc/en/Reference/StringLength
 * mbed code: boolean isStringEmpty(...) { ... }
 *               loop { isStringEmpty(X) }
 * @param {!Blockly.Block} block Block to generate the code from.
 * @return {array} Completed code with order of operation.
 */
Blockly.mbed['text_isEmpty'] = function(block) {
  var func = [];
  func.push('boolean ' + Blockly.mbed.DEF_FUNC_NAME + '(String msg) {');
  ...
  func.push('  }');
  func.push('}');
  var funcName = Blockly.mbed.addFunction('isStringEmpty', func.join('\n'));
  var argument0 = Blockly.mbed.valueToCode(block, 'VALUE',
      Blockly.mbed.ORDER_UNARY_POSTFIX);
  if (argument0 == '') {
    argument0 = '""';
 ...
 * String trim info: http://mbed.cc/en/Tutorial/StringLengthTrim
 * mbed code: loop { String(X).trim() }
 * @param {!Blockly.Block} block Block to generate the code from.
 * @return {array} Completed code with order of operation.
 */
Blockly.mbed['text_trim'] = function(block) {
  // Trim spaces.
 ...
 * Serial info: http://mbed.cc/en/Reference/Serial
 * mbed code: setup { Serial.begin(9600);     }
 *               loop  { Serial.print(String(X)) }
 * @param {!Blockly.Block} block Block to generate the code from.
 * @return {string} Completed code.
 */
Blockly.mbed['text_print'] = function(block) {
  var serialId = Blockly.mbed.Boards.selected.serial[0][1];
  var setupCode = serialId + '.begin(9600);';
  Blockly.mbed.addSetup('serial_' + serialId, setupCode, false);
  var argument0 = Blockly.mbed.valueToCode(block, 'TEXT',
 ...
 * Serial info: http://mbed.cc/en/Reference/Serial
 * mbed code: getUserInputPrompt(...) { ... }
 *               loop { getUserInputPrompt("X")) }
 * @param {!Blockly.Block} block Block to generate the code from.
 * @return {array} Completed code with order of operation.
 */
Blockly.mbed['text_prompt_ext'] = function(block) {
  // Get the first Serial peripheral of mbed board
  var serialId = Blockly.mbed.Boards.selected.serial[0][1];
  ...
  if (toNumber) {
    func.push('int ' + Blockly.mbed.DEF_FUNC_NAME + '(String msg) {');
  } else {
    func.push('String ' + Blockly.mbed.DEF_FUNC_NAME + '(String msg) {');
  }
  ...
  func.push('}');
  var funcName = Blockly.mbed.addFunction(
  ...
  var msg = Blockly.mbed.valueToCode(block, 'TEXT',
      Blockly.mbed.ORDER_NONE) || '""';
 ...
 * been removed from the toolbox and not used for mbed code.      *
 * ***************************************************************** */
Blockly.mbed['text_endString'] = function(block) {
  return ['', Blockly.mbed.ORDER_UNARY_POSTFIX];
};

Blockly.mbed['text_indexOf'] = function(block) {
  return ['', Blockly.mbed.ORDER_UNARY_POSTFIX];
};

Blockly.mbed['text_charAt'] = function(block) {
  return ['', Blockly.mbed.ORDER_UNARY_POSTFIX];
};

Blockly.mbed['text_getSubstring'] = function(block) {
  return ['', Blockly.mbed.ORDER_UNARY_POSTFIX];
};

Blockly.mbed['text_changeCase'] = function(block) {
  return ['', Blockly.mbed.ORDER_UNARY_POSTFIX];
};

Blockly.mbed['text_prompt'] = function(block) {
  return ['', Blockly.mbed.ORDER_UNARY_POSTFIX];
};
